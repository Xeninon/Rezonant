...
const selectChirpsByUser = `-- name: SelectChirpsByUser :many
SELECT id, created_at, updated_at, body, user_id FROM chirps
WHERE user_id = $1
ORDER BY created_at ASC
`

func (q *Queries) SelectChirpsByUser(ctx context.Context, userID uuid.UUID) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, selectChirpsByUser, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
